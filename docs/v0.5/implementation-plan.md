...
- v0.5-spike 4 検証（`docs/v0.5/spike-plan.md`）が完了済み
- S1 GO 状態（image bridge 採用可能） / S2 採用パターン確定 / S3 toolkit 判断確定 / S4 PCM 判断確定
- 旦那様の最終 GO
...
**A. toolkit 採用パターン（S3 GO 時）**
1. `pnpm add even-toolkit upng-js`（package.json 1 行追加）
2. `src/even/bridge.ts` で `import { cleanForG2 } from 'even-toolkit/glasses/text-clean'`
3. `render()` 内で全 text container の content を `cleanForG2(content)` に通す（~10 行追加）

**B. 自前実装パターン（S3 NO-GO 時）**
1. **新規 `src/even/text-clean.ts`**（約 30 行）
   - emoji 範囲（U+1F600〜U+1F64F 等）を削除
   - 未サポート文字（ASCII 範囲外 + CJK 漢字以外）を `?` に置換
   - toolkit の cleanForG2 と同等の挙動を移植
2. `src/even/bridge.ts` で同様に適用

**完了条件**:
- emoji 入りメッセージを送っても SDK エラーなし
- 既存 ASCII / CJK テキストは変化なし
