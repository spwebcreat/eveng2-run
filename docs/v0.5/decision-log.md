...
| 2026-05-28 | v0.5-spike 実機検証完了: S1 / S4 ともに **GO 確定**（Section 8 参照） |

---

## 8. v0.5-spike 実機検証結果（2026-05-28 確定）

### S1 (2×2 タイル image 表示) — ✅ **GO 確定**

- 検証 ehpk: `spike-builds/g2-run-hud-spike-s1-0.5.2.ehpk` (SHA256: `948d6c3f...681b594e`)
- 判定: 3 パターン (A 境界線 / B グラデーション / C 大型タイポ) 全て OK（旦那様視覚チェック）
- 結論: **v0.6 image HUD 路線継続可能**
- 次タスク: S2 (image update p95 latency 計測) を v0.6 着手前に必須実施

### S2 (image update latency 計測) — ⏸ 未着手

- v0.6 着手前に必須
- S1 GO のため、当初プラン通り「1Hz / 状態変化 / 200ms × 30 分連続」を実施

### S3 (even-toolkit ビルド検証) — ✅ 採用確定（既存）

- `cleanForG2Safe` 経由で `even-toolkit/text-clean` を v0.5-6 に取り込み（P0-1 で対応済）
- `png-utils` は v0.6 で再評価。`upng-js` は不要

### S4 (G2 マイク PCM stream) — ✅ **GO 確定**

- 検証 ehpk: `spike-builds/g2-run-hud-spike-s4-0.5.3.ehpk` (SHA256: `fab1695b...980529ca`)
- 5 分間計測結果 (旦那様実機):
  - 完走: `done (timeout)` で自然終了
  - events: **n=2979** (5 分間で 2979 audio events)
  - 平均 throughput: **31,776 B/s** （32,000 ± 10% の理論値ど真ん中）
  - 平均 interval: **~101ms** (理論値 100ms にほぼ一致)
  - bytes/event: **~3,200** (理論値どおり)
  - first event latency: 数秒以内に STATUS 表示開始 (< 1000ms 想定)
  - stream 安定性: 5 分間欠落なし
  - グラス表示: `MIC NNN B/s NNN ms ago | n=NN t=NNs` 継続表示確認
- 結論: **v0.9 音声コマンド実装着手可能**。Whisper-tiny on-device STT も視野に入る

### v0.5 本実装着手判定

S3 採用確定 + P0 修正 5 件完了済 → **着手可能**。`docs/v0.5/implementation-plan.md` 通り進める。

### spike 過程で発覚した重要事故 (learning P1 記録済)

- 初回 ehpk (v0.5.0 / v0.5.1) は vite multi-entry build で本番 entry が dist/ に混入し、Hub OS が本番 UI を起動する事故が発生
- Codex 第二意見取得 + clean build 方針 + dist/ 検査ゲート 4 項目 で再発防止
- 詳細: `.claude/rules/learning/core.md` の P1 追記 5 件 (2026-05-27)、および `docs/v0.5/spike-results/codex-brief.md`

### 後処理

- Hub Portal の spike 用 2 アプリ削除（旦那様作業）
- `spike-builds/` の新 ehpk は git ignored、検証完了後も短期保持
